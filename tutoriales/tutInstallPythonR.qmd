...
```bash
sudo apt update
sudo apt install -y r-base r-base-dev r-recommended r-cran-renv
```
...
renv::init()
```
    # Debe mostrar la versión de R recién instalada
```bash
renv::install(c("DiagrammeR", "reticulate", "kableExtra", "tidyverse", "knitr", "cowplot", "ggfx", "rstatix", "languageserver", "bibliometrix"))
