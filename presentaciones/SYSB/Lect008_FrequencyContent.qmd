...
$$\cos(\alpha) = 1 - \frac{\alpha^2}{2!} + \frac{\alpha^4}{4!} - \frac{\alpha^6}{6!} + \dots$$
...
We must apply the powers of the imaginary unit $i$:
* $i^2 = -1$
* $i^3 = -i$
* $i^4 = 1$
* $i^5 = i$
...
1.  The Real part is exactly $\cos(\alpha)$.
2.  The Imaginary part is exactly $\sin(\alpha)$.

**Final Identity:**
$$e^{i\alpha} = \cos(\alpha) + i \sin(\alpha)$$

::: {.notes}
This identity is essential for Fourier analysis and processing biomedical signals like EMG or ECG.
:::
...
1. **Magnitude ($r$):** The distance from the origin.
   $$r = |z| = \sqrt{a^2 + b^2}$$
2. **Phase ($\alpha$):** The angle from the positive real axis.
   $$\alpha = \arg(z) = \tan^{-1}\left(\frac{b}{a}\right)$$

---

## From Trigonometry to Euler

From the geometry of a right triangle, we know:
* $a = r \cos(\alpha)$
* $b = r \sin(\alpha)$

Substituting these into $z = a + bi$:
$$z = r \cos(\alpha) + i(r \sin(\alpha))$$
$$z = r (\cos(\alpha) + i \sin(\alpha))$$

Using **Euler's Formula** ($e^{i\alpha} = \cos\alpha + i\sin\alpha$), we simplify this to:
$$z = r e^{i\alpha}$$
...
**2. Division:**
$$\frac{z_1}{z_2} = \frac{r_1}{r_2} e^{i(\alpha_1 - \alpha_2)}$$

**3. Signal Analysis:**
It allows us to represent a rotating phasor, which is the basis for the Fourier Transform.
...
2. **Calculate Angle:**
   $$\alpha = \tan^{-1}\left(\frac{1}{1}\right) = 45^\circ = \frac{\pi}{4} \text{ rad}$$
...
where
$$
y_1(t) = \frac{A}{2} e^{j\phi} \int_{-\infty}^{\infty} h(\tau)\, e^{j\omega_0(t-\tau)}\, d\tau
$$
$$
y_2(t) = \frac{A}{2} e^{-j\phi} \int_{-\infty}^{\infty} h(\tau)\, e^{-j\omega_0(t-\tau)}\, d\tau
$$
...
Then,
$$
y_1(t) = e^{j\phi} e^{j\omega_0 t} H_1
$$

Similarly,
$$
y_2(t) = e^{-j\phi} e^{-j\omega_0 t} H_1
$$
...
1. **Symmetry**: if the signal is real, then
   $$
   X(-\omega)=X^*(\omega)
   $$
2. **Periodicity** in discrete time:
   $$
...
X(\omega+2\pi)
=
\int_{-\infty}^{\infty} x(t)e^{-j(\omega+2\pi)t}\,dt
=
\int_{-\infty}^{\infty} x(t)e^{-j\omega t}e^{-j2\pi t}\,dt
$$
...
=
2\cos\left(\frac{\omega}{2}\right)e^{-j\omega/2}
$$
...
digraph G {
  ...
  node [shape=box, style="rounded,filled", fillcolor="lightgray", color="gray20", fontname="Helvetica"];
  edge [color="gray30", arrowsize=0.8];

  A [label="Real signal"];
  B [label="Conjugate symmetry:\nX(-ω)=X*(ω)"];
  C [label="Discrete-time signal"];
  D [label="Periodicity:\nX(e^{j(ω+2π)})=X(e^{jω})"];
  E [label="Real discrete-time signal"];
  F [label="Both properties:\nsymmetric and periodic"];
