...
## Review: FT (Continuous) vs. DTFT (Discrete)
...
**Frequency Variable:**
$\Omega$ (Continuous frequency, [rad/s])

. . .

**Spectrum Nature:**
The spectrum $X(j\Omega)$ is:
* **Continuous**
* **Aperiodic**
...
* **Formula:** Use the warping relationship:
    $$\Omega_p = \frac{2}{T} \tan\left(\frac{\omega_p T}{2}\right)$$   $$\Omega_s = \frac{2}{T} \tan\left(\frac{\omega_s T}{2}\right)$$
...
- **Transformation:** Replace $s_{norm}$ with $s / \Omega_c$.
$$s_{norm} \to \frac{s}{\Omega_c}$$
- **Denormalized Transfer Function:**
$$H_{LP}(s) = H_{norm}\left(\frac{s}{\Omega_c}\right)$$
...
- **Transformation:** Replace $s_{norm}$ with $\Omega_c / s$.
$$s_{norm} \to \frac{\Omega_c}{s}$$
- **Denormalized Transfer Function:**
$$H_{HP}(s) = H_{norm}\left(\frac{\Omega_c}{s}\right)$$
...
- **Transformation:** Replace $s_{norm}$ with $\frac{s^2 + \Omega_0^2}{B s}$.
$$s_{norm} \to \frac{s^2 + \Omega_0^2}{B s}$$
- **Parameters:**
    - $\Omega_0 = \sqrt{\Omega_1 \Omega_2}$ (geometric mean)
    - $B = \Omega_2 - \Omega_1$ (bandwidth)
    ($\Omega_1, \Omega_2$ are the desired band edges in rad/s)
- **Denormalized Transfer Function:**
$$H_{BP}(s) = H_{norm}\left(\frac{s^2 + \Omega_0^2}{B s}\right)$$

## Normalized Lowpass to Denormalized Bandstop

- **Goal:** Transform a normalized LP ($\omega_c = 1$) to a denormalized BS with center frequency $\Omega_0$ and bandwidth $B$.
- **Transformation:** Replace $s_{norm}$ with $\frac{B s}{s^2 + \Omega_0^2}$.
$$s_{norm} \to \frac{B s}{s^2 + \Omega_0^2}$$
- **Parameters:**
    - $\Omega_0 = \sqrt{\Omega_1 \Omega_2}$
    - $B = \Omega_2 - \Omega_1$
- **Denormalized Transfer Function:**
$$H_{BS}(s) = H_{norm}\left(\frac{B s}{s^2 + \Omega_0^2}\right)$$
