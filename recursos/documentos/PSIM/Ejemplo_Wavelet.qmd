...
Desde el punto de vista de ingeniería biomédica, la elección de la wavelet puede justificarse mediante varios criterios complementarios.

La **ortogonalidad** o, en algunos casos, la **biortogonalidad**, favorece una reconstrucción estable y una interpretación más limpia de los coeficientes. La documentación oficial de PyWavelets permite consultar explícitamente estas propiedades para cada wavelet discreta disponible.

La **simetría** es relevante porque reduce distorsiones de fase y favorece una localización temporal más interpretable de puntos fiduciales. La literatura clásica sobre detección de QRS reportó mejores resultados con wavelets que exhiben simetría y compacidad temporal.

La **longitud del filtro** y, por extensión, el soporte temporal efectivo, afectan la resolución temporal y los efectos de borde. Filtros demasiado largos suavizan más, pero pueden perder precisión temporal en transitorios estrechos como el complejo QRS.

El **número de momentos nulos** es útil porque permite separar mejor tendencias suaves y componentes abruptas. No obstante, aumentar momentos nulos suele implicar filtros más largos, de modo que existe una compensación entre discriminación espectral y localización temporal.

Finalmente, si el problema es electrocardiografía, debe incluirse un criterio **empírico vinculado a la señal**. En este reporte se utiliza un criterio sencillo y reproducible: la proporción de energía que la wavelet concentra en los niveles de detalle que, para una señal muestreada a 360 Hz, son plausibles para el complejo QRS.
